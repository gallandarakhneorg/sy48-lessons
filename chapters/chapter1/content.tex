...
\begin{frame}<6>{{Machine-Independent Code} Optimizer}
	...
	\begin{minipage}{.8\linewidth}
		...
		\vspace{.5cm}
		\begin{itemize}
		\item Modern compilers include a machine-independent code optimizer
		\item Compilers that spent a large amount of time on this phase are named ``optimizing compilers''
		\end{itemize}
		\vspace{.5cm}
		\begin{block}{Note}
...
\begin{frame}{Tools to Create a Compiler}
	...
	\vspace{.5cm}
	\fancybox{Scanner generators}{Lexical analyzers from a regex description tokens \\
