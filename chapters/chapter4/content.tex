	...
	\begin{footnotesize}
		...
		\begin{block}{Static Size Determination}
			\begin{itemize}
				...
				\item Data of varying length (string, dynamic array\dots) is associated to a pointer (usually 64 or 128 bits) to the fixed storage unit for that data
			\end{itemize}
		\end{block}
...
\begin{frame}{{First Form:} Type Synthesis}
	...
	\item Relationship between types must be defined, e.g., $T_1 \subset T_2$, value set $T_1$ is included value set $T_2$
	\end{itemize}
	\vspace{.5cm}
	\begin{example}
		\begin{itemize}
...
\begin{frame}{Example of Type Conversions in SDD}
	...
	\begin{sdd}
		\sddprod{E}{\sddpn{E} \; \sddts{+} \; \sddpn{E}}{
			...
			o1 = \alert{widenVar}(E$_1$.addr, head.type, E$_1$.type) \newl
			o2 = \alert{widenVar}(E$_2$.addr, head.type, E$_2$.type) \newl
			head.addr = \kw{new} TemporaryVariable() \newl
			\kw{quadruple}("+", o1, o2, head.addr)
		}
		\sddprod{E}{\sddts{id} \; \sddts{=} \; \sddpn{E}}{
			v = SymbolTable.current.get(\sddts{id}.lexeme) \newl
			head.addr = v.addr \newl
			head.type = v.type \newl
			w = \alert{narrowVar}(E.addr, head.type, E.type) \newl
			\kw{if} (w $\neq$ E.addr) \newl
			\hspace{2em}\kw{warning}~"May loose information" \newl
			\kw{else} \newl
			\hspace{2em}w = \alert{widenVar}(E.addr, head.type, E.type) \newl
