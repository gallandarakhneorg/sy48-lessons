	...
	\begin{columns}
		...
		\begin{column}{.7\linewidth}\begin{small}
				...
				\only<3>{\begin{itemize}
						...
						\item Area where these data are stored is named \emph{Static Data}, usually put just after the Code area
...
\begin{frame}{Structure of the Activation Record}
	...
	\begin{columns}
		...
		\begin{column}{.7\linewidth}\begin{small}
				...
				\end{itemize}}
				\only<5>{\begin{itemize}
						\item ``Access link'' to locate data needed by the called procedure and that are located elsewhere (in another activation record\dots)
						\item Explained in following slides
				\end{itemize}}
				\only<6>{\begin{itemize}
				...
				\only<7>{\begin{itemize}
						...
						\item We may prefer to place that value in a processor register for efficiency
				\end{itemize}}
				\only<8>{\begin{itemize}
						\item Actual parameters are given by the caller and used by the callee procedure
						\item Commonly, these values are not placed in the activation record but rather in processor registers, when possible
				\end{itemize}}
			\end{small}
		\end{column}
	\end{columns}
...
\begin{frame}[background=6]{Typical Return Sequence}
	\begin{enumerate}
		...
		\item Callee branches to the return address that the caller placed in the status field
...
\begin{frame}{Maintaining the Displays when Returning from Records}
	\begin{myalgorithm}
		\Inputs{Stack $s$; called procedure $p$; nesting depth of $p$ is $N_p$}
		\Begin{
			$d[N_p]$ \affect \textcircled{P} of $p$
		}
	\end{myalgorithm}
\end{frame}
...
\begin{frame}{Optimization Policy}
	...
	\begin{example}
		\begin{itemize}
			\item Visiting a large amount of data and performing small operations on it is not a good approach for CPU (but good for GPU)
...
\begin{frame}{Deallocation and Coalescing Free Space}
	\begin{block}{Deallocation of a Chunk}
...
\begin{frame}{{How Finding} Unreachable Objects?}
	\begin{block}{Approach \#1}
		...
		Reference counting is an approximation of this approach:
		\begin{itemize}
